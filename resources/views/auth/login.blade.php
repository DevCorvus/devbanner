...
<main class="w-5/6 md:w-2/4 lg:w-1/2 mx-auto rounded-xl p-4 text-black" style="background-color: #243b55">
  ...
  <form action="{{ route('admin.login') }}" method="POST">
    ...
    <div>
      ...
      <input
        class="w-full p-2 rounded-sm ring ring-transparent @error('email') ring-red-400 @enderror focus:ring-blue-300 focus:outline-none"
        type="text"
        name="email"
        placeholder="Email"
        value="{{ old('email') }}"
    ...
    <div class="mt-1">
      ...
      <input
        class="w-full p-2 rounded-sm ring ring-transparent @error('password') ring-red-400 @enderror focus:ring-blue-300 focus:outline-none"
        type="password"
        name="password"
